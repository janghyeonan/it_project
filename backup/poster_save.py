...
def csv_read_list(year):
    import csv
    import os
    data = csv.reader(open('/home/itwill02/project/data/mdict'+str(year)+'.csv', 'r'))
    mnum=[]
    for i in data:
        mnum.append(i)        
    mdict =[mnum[i][0] for i in range(len(mnum))]    
    mdict_s = []    
    for j in (os.listdir('/home/itwill02/project/data/poster/')):
...
def posterSucker(mn):
    driver = webdriver.PhantomJS("/home/itwill02/project/phantomjs")    
    ...
    try:
        ...
        try:
            with urlopen(soup.find('img',id='targetImage')['src']) as f:
                with open('/home/itwill02/project/data/poster/' + str(mn)+'.jpg', 'wb') as w:
                    img = f.read() 
                    w.write(img)
